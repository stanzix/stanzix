  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        {examples.map((e, i) => (
          <Card key={i} highlight={approvedExamples.has(i)} style={approvedExamples.has(i) ? { boxShadow: "0 0 20px rgba(184,134,78,0.15)", border: "1px solid rgba(184,134,78,0.4)", transition: "all 0.4s ease" } : { transition: "all 0.4s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px" }}>
              <span style={{ fontSize: "12px", fontFamily: "'IBM Plex Mono', monospace", color: "rgba(255,255,255,0.3)" }}>Example {i + 1}</span>
            ...
            <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: "6px", padding: "10px 12px", marginBottom: "8px" }}>
              ...
              {editKey === `${i}.userMessage` ? (
                ...
                <div
                  style={{ color: "#F0EBE0", fontSize: "13px", cursor: "text" }}
                  title="Click to edit"
                  onClick={ev => startEdit(`${i}.userMessage`, e.userMessage, ev)}
                >{e.userMessage}</div>
              )}
            </div>
            <div style={{ background: "rgba(184,134,78,0.05)", borderRadius: "6px", padding: "10px 12px" }}>
              <div style={{ fontSize: "11px", color: "rgba(184,134,78,0.6)", marginBottom: "4px" }}>ASSISTANT</div>
              {editKey === `${i}.idealResponse` ? (
                <textarea
                  value={editVal}
                  autoFocus
                  rows={4}
                  style={{ ...INPUT_STYLE, fontSize: "13px", lineHeight: "1.5", background: "rgba(184,134,78,0.07)", color: "rgba(255,255,255,0.8)" }}
                  onChange={ev => setEditVal(ev.target.value)}
                  onBlur={commitEdit}
                  onKeyDown={handleTextareaKeyDown}
                  onClick={ev => ev.stopPropagation()}
