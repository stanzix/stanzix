...
export const Badge = ({ children, active, onClick }) => (
  <button onClick={onClick} style={{
    ...
    border: active ? "1.5px solid #C07A56" : "1.5px solid rgba(255,255,255,0.1)",
    background: active ? "rgba(192,122,86,0.15)" : "rgba(255,255,255,0.03)",
    color: active ? "#C07A56" : "rgba(255,255,255,0.6)",
...
export const Card = ({ children, style, highlight, ...rest }) => (
  <div style={{
    background: highlight ? "rgba(192,122,86,0.06)" : "rgba(255,255,255,0.03)",
    border: highlight ? "1px solid rgba(192,122,86,0.3)" : "1px solid rgba(255,255,255,0.06)",
...
export const Btn = ({ children, onClick, primary, disabled, small, style: s }) => (
  <button onClick={onClick} disabled={disabled} style={{
    ...
    background: primary
      ? disabled ? "rgba(192,122,86,0.3)" : "linear-gradient(135deg, #C07A56, #A96A49)"
  ...
  return (
    <div style={{ marginBottom: sub ? "8px" : "16px" }}>
      <Tag htmlFor={htmlFor} style={{
        ...
        color: sub ? "rgba(255,255,255,0.55)" : "#C07A56",
...
export const StepExample = ({ children, onDismiss }) => (
  <div style={{ background: "rgba(192,122,86,0.04)", border: "1px solid rgba(192,122,86,0.15)", borderRadius: "8px", padding: "10px 14px", fontSize: "12px", color: "rgba(255,255,255,0.5)", lineHeight: 1.6, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "8px", marginBottom: "16px" }}>
...
export const LoadingIndicator = () => {
  ...
  return (
    <div role="status" aria-live="polite" style={{ padding: "20px 24px", marginBottom: "16px", background: "linear-gradient(135deg, rgba(192,122,86,0.08), rgba(192,122,86,0.03))", border: "1px solid rgba(192,122,86,0.2)", borderRadius: "12px", display: "flex", alignItems: "center", gap: "16px" }}>
      <div style={{ width: "40px", height: "40px", borderRadius: "12px", background: "rgba(192,122,86,0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, animation: "glow 2s ease-in-out infinite" }}>
        <Loader2 size={20} color="#C07A56" className="spin" />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: "14px", fontWeight: 600, color: "#C07A56", fontFamily: "'Libre Franklin', sans-serif", marginBottom: "4px" }}>{LOADING_PHRASES[phraseIdx]}</div>
      ...
      <div style={{ width: "60px", height: "4px", borderRadius: "2px", background: "rgba(255,255,255,0.06)", overflow: "hidden", flexShrink: 0 }}>
        <div style={{ width: "40%", height: "100%", borderRadius: "2px", background: "linear-gradient(90deg, #C07A56, #A96A49)", animation: "shimmerBar 1.5s ease-in-out infinite" }} />
      </div>
    </div>
  );
};
...
export const Fireworks = () => {
  ...
  const [particles] = useState(() => prefersReduced ? [] : Array.from({ length: 30 }, (_, i) => {
    ...
    const colors = ["#C07A56", "#f0c674", "#A96A49", "#ffd700", "#ff9500", "#7DB86A"];
  ...
  if (prefersReduced) {
    return (
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, pointerEvents: "none", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: "18px", fontWeight: 700, color: "#C07A56", fontFamily: "'Libre Franklin', sans-serif", background: "rgba(12,11,10,0.9)", padding: "12px 24px", borderRadius: "10px", border: "1px solid rgba(192,122,86,0.3)" }}>Instructions Copied!</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, pointerEvents: "none", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ fontSize: "18px", fontWeight: 700, color: "#C07A56", fontFamily: "'Libre Franklin', sans-serif", textShadow: "0 0 20px rgba(192,122,86,0.5)", animation: "fadeInScale 0.4s 0.2s ease forwards", opacity: 0 }}>Instructions Copied!</div>
    </div>
  );
};
